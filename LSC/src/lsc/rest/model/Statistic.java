...
public class Statistic extends Base {
	...
	*/
	...
	public StatisticData _sum() {
		...
		sum._initValues(Float.valueOf(0));
		if(datas == null) {
			return sum;
		}
			...
			for(Goal.Function f : Goal.Function.values())
				for(Goal.Reference r : Goal.Reference.values())
					for(Goal.Perc p : Goal.Perc.values()) {
						Float now = d._getValue(f,r,p);
						Float since = sum._getValue(f,r,p);
						sum._setValue(f,r,p, since + now );
					}
		return sum;
	}
	...
	public StatisticData _average() {
		...
		avg._initValues(Float.valueOf(0));
		if(datas == null) {
			return avg;
		}
		for(Goal.Function f : Goal.Function.values())
			for(Goal.Reference r : Goal.Reference.values())
				for(Goal.Perc p : Goal.Perc.values()) {
					Float s = sum._getValue(f,r,p);
					avg._setValue(f,r,p, s / datas.size() );
				}
		return avg;
	}
		...
		for(StatisticData d : datas)
			for(Goal.Function f : Goal.Function.values())
				for(Goal.Reference r : Goal.Reference.values())
					for(Goal.Perc p : Goal.Perc.values()) {
						Float now = d._getValue(f,r,p);
						Float since = max._getValue(f,r,p);
						if( since==null || now > since)
							max._setValue(f,r,p, now );
					}
		return max;
	}
		...
		for(StatisticData d : datas)
			for(Goal.Function f : Goal.Function.values())
				for(Goal.Reference r : Goal.Reference.values())
					for(Goal.Perc p : Goal.Perc.values()) {
						Float now = d._getValue(f,r,p);
						Float since = min._getValue(f,r,p);
						if( since==null || now < since)
							min._setValue(f,r,p, now );
					}
		return min;
	}
