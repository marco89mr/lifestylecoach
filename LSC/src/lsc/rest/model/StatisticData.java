...
public class StatisticData extends Base {
	...
	/*
	@XmlTransient
	public Map<Goal.Function, Map<Goal.Reference, Map<Goal.Perc, Float> > > _getValues() {
	...
	public void _setValues(Map<Goal.Function, Map<Goal.Reference, Map<Goal.Perc, Float> > > values) {
	...
	*/
	...
	public Float _getValue(Goal.Function f, Goal.Reference r, Goal.Perc p) {

		if(f==Function.average && r==Reference.target && p==Perc.abs)
			return average_target_abs;
		else if(f==Function.average && r==Reference.target && p==Perc.perc)
			return average_target_perc;
		else if(f==Function.average && r==Reference.increment && p==Perc.abs)
			return average_increment_abs;
		else if(f==Function.average && r==Reference.increment && p==Perc.perc)
			return average_increment_perc;

		else if(f==Function.sum && r==Reference.target && p==Perc.abs)
			return sum_target_abs;
		else if(f==Function.sum && r==Reference.target && p==Perc.perc)
			return sum_target_perc;
		else if(f==Function.sum && r==Reference.increment && p==Perc.abs)
			return sum_increment_abs;
		else if(f==Function.sum && r==Reference.increment && p==Perc.perc)
			return sum_increment_perc;

		else if(f==Function.max && r==Reference.target && p==Perc.abs)
			return max_target_abs;
		else if(f==Function.max && r==Reference.target && p==Perc.perc)
			return max_target_perc;
		else if(f==Function.max && r==Reference.increment && p==Perc.abs)
			return max_increment_abs;
		else if(f==Function.max && r==Reference.increment && p==Perc.perc)
			return max_increment_perc;

		else if(f==Function.min && r==Reference.target && p==Perc.abs)
			return min_target_abs;
		else if(f==Function.min && r==Reference.target && p==Perc.perc)
			return min_target_perc;
		else if(f==Function.min && r==Reference.increment && p==Perc.abs)
			return min_increment_abs;
		else if(f==Function.min && r==Reference.increment && p==Perc.perc)
			return min_increment_perc;

		else if(f==Function.last && r==Reference.target && p==Perc.abs)
			return last_target_abs;
		else if(f==Function.last && r==Reference.target && p==Perc.perc)
			return last_target_perc;
		else if(f==Function.last && r==Reference.increment && p==Perc.abs)
			return last_increment_abs;
		else if(f==Function.last && r==Reference.increment && p==Perc.perc)
			return last_increment_perc;

		else if(f==Function.first && r==Reference.target && p==Perc.abs)
			return first_target_abs;
		else if(f==Function.first && r==Reference.target && p==Perc.perc)
			return first_target_perc;
		else if(f==Function.first && r==Reference.increment && p==Perc.abs)
			return first_increment_abs;
		else if(f==Function.first && r==Reference.increment && p==Perc.perc)
			return first_increment_perc;
		
		System.out.println("ERROR StatisticData._setValue"+f+r+p);
		return null;
	}
	
	public void _setValue(Goal.Function f, Goal.Reference r, Goal.Perc p, Float value) {
		if(value == null)
			value = Float.valueOf(0);
		if(f==Function.average && r==Reference.target && p==Perc.abs)
			average_target_abs = value;
		else if(f==Function.average && r==Reference.target && p==Perc.perc)
			average_target_perc = value;
		else if(f==Function.average && r==Reference.increment && p==Perc.abs)
			average_increment_abs = value;
		else if(f==Function.average && r==Reference.increment && p==Perc.perc)
			average_increment_perc = value;

		else if(f==Function.sum && r==Reference.target && p==Perc.abs)
			sum_target_abs = value;
		else if(f==Function.sum && r==Reference.target && p==Perc.perc)
			sum_target_perc = value;
		else if(f==Function.sum && r==Reference.increment && p==Perc.abs)
			sum_increment_abs = value;
		else if(f==Function.sum && r==Reference.increment && p==Perc.perc)
			sum_increment_perc = value;

		else if(f==Function.max && r==Reference.target && p==Perc.abs)
			max_target_abs = value;
		else if(f==Function.max && r==Reference.target && p==Perc.perc)
			max_target_perc = value;
		else if(f==Function.max && r==Reference.increment && p==Perc.abs)
			max_increment_abs = value;
		else if(f==Function.max && r==Reference.increment && p==Perc.perc)
			max_increment_perc = value;

		else if(f==Function.min && r==Reference.target && p==Perc.abs)
			min_target_abs = value;
		else if(f==Function.min && r==Reference.target && p==Perc.perc)
			min_target_perc = value;
		else if(f==Function.min && r==Reference.increment && p==Perc.abs)
			min_increment_abs = value;
		else if(f==Function.min && r==Reference.increment && p==Perc.perc)
			min_increment_perc = value;

		else if(f==Function.last && r==Reference.target && p==Perc.abs)
			last_target_abs = value;
		else if(f==Function.last && r==Reference.target && p==Perc.perc)
			last_target_perc = value;
		else if(f==Function.last && r==Reference.increment && p==Perc.abs)
			last_increment_abs = value;
		else if(f==Function.last && r==Reference.increment && p==Perc.perc)
			last_increment_perc = value;

		else if(f==Function.first && r==Reference.target && p==Perc.abs)
			first_target_abs = value;
		else if(f==Function.first && r==Reference.target && p==Perc.perc)
			first_target_perc = value;
		else if(f==Function.first && r==Reference.increment && p==Perc.abs)
			first_increment_abs = value;
		else if(f==Function.first && r==Reference.increment && p==Perc.perc)
			first_increment_perc = value;
		else
			System.out.println("ERROR StatisticData._setValue"+f+r+p);
	}
	...
	public void _initValues(Float x) {
		for(Function f : Function.values()){
			for(Reference r : Reference.values()){
				for(Perc p : Perc.values()){
					_setValue(f, r, p, x);
