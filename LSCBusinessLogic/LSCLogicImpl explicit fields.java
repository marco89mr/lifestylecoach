...
public class LSCLogicImpl implements LSCLogic {
	...
	public Statistic computeCompleteStatistic(	int user_id,
												String record_type,
												String data_name,
												String from,
												String to,
												Interval on_interval	) {
		System.out.println("---> computeCompleteStatistic");
	...
	public StatisticData computeStatisticData(	int user_id,
												String record_type,
												String data_name,
												String from,
												String to,
		...
		if( old==null )
			old = computePartialStatisticData(	user_id,
												record_type,
												data_name,
												from,
												to	);
		Float old_average 	= old.getAverageTargetAbs();
		Float old_sum 		= old.getSumTargetAbs();
		Float old_max 		= old.getMaxTargetAbs();
		Float old_min 		= old.getMinTargetAbs();
		Float old_last 		= old.getLastTargetAbs();
		Float old_first 	= old.getFirstTargetAbs();
		// compute statistic datas
		StatisticData statisticData = new StatisticData();
		...
		statisticData.setAverageTargetAbs( average );
		statisticData.setSumTargetAbs( sum );
		statisticData.setMaxTargetAbs( max );
		statisticData.setMinTargetAbs( min );
		statisticData.setLastTargetAbs( last );
		statisticData.setFirstTargetAbs( first );
		//
		//elaborate TARGET PERC values
		Float average_target_perc 	= average / old_average * 100;
		Float sum_target_perc 		= sum / old_sum * 100;
		Float max_target_perc 		= max / old_max * 100;
		Float min_target_perc 		= min / old_min* 100;
		Float last_target_perc 		= last / old_last * 100;
		Float first_target_perc 	= first / old_first * 100;
		statisticData.setAverageTargetPerc( average_target_perc );
		statisticData.setSumTargetPerc( sum_target_perc );
		statisticData.setMaxTargetPerc( max_target_perc );
		statisticData.setMinTargetPerc( min_target_perc );
		statisticData.setLastTargetPerc( last_target_perc );
		statisticData.setFirstTargetPerc( first_target_perc );
		//
		//elaborate INCREMENT ABS values
		Float average_increment_abs 	= average - old_average;
		Float sum_increment_abs 		= sum - old_sum;
		Float max_increment_abs 		= max - old_max;
		Float min_increment_abs 		= min - old_min;
		Float last_increment_abs 		= last - old_last;
		Float first_increment_abs 		= first - old_first;
		statisticData.setAverageIncrementAbs( average_increment_abs );
		statisticData.setSumIncrementAbs( sum_increment_abs );
		statisticData.setMaxIncrementAbs( max_increment_abs );
		statisticData.setMinIncrementAbs( min_increment_abs );
		statisticData.setLastIncrementAbs( last_increment_abs );
		statisticData.setFirstIncrementAbs( first_increment_abs );
		//
		//elaborate INCREMENT PERC values
		Float average_increment_perc 	= average_increment_abs / old_average * 100;
		Float sum_increment_perc 		= sum_increment_abs / old_sum * 100;
		Float max_increment_perc 		= max_increment_abs / old_max * 100;
		Float min_increment_perc 		= min_increment_abs / old_min* 100;
		Float last_increment_perc 		= last_increment_abs / old_last * 100;
		Float first_increment_perc 	= first / old_first * 100;
		statisticData.setAverageIncrementPerc( average_increment_perc );
		statisticData.setSumIncrementPerc( sum_increment_perc );
		statisticData.setMaxIncrementPerc( max_increment_perc );
		statisticData.setMinIncrementPerc( min_increment_perc );
		statisticData.setLastIncrementPerc( last_increment_perc );
		statisticData.setFirstIncrementPerc( first_increment_perc );
		//
		return statisticData;
	}
    

	public StatisticData computePartialStatisticData(	int user_id,
														String record_type,
														String data_name,
														String from,
														String to		) {
		...
		statisticData.setFromDate( from );
		statisticData.setToDate( to );
		statisticData.setValues( new HashMap<Goal.Function, Map<Goal.Reference, Map<Goal.Perc, Float>>>() );
		//
		//elaborate TARGET ABS values
		for(Goal.Function f : Goal.Function.values()){
			float x = elaborateDataCollection(Function.average, data_collection);
			
			Map<Goal.Perc, Float> m = new HashMap<Goal.Perc, Float>();
			m.put(Goal.Perc.abs, x);
			
			Map<Goal.Reference, Map<Goal.Perc, Float>> mm = new HashMap<Goal.Reference, Map<Goal.Perc, Float>>();
			mm.put(Goal.Reference.target, m);
			
			statisticData.getValues().put(f, mm);
		}
		Float average 	= elaborateDataCollection(Function.average, data_collection);
		Float sum 		= elaborateDataCollection(Function.sum, data_collection);
		Float max 		= elaborateDataCollection(Function.max, data_collection);
	...
	public Float elaborateDataCollection(Function function, DataCollection data_collection) {
		switch(function){
		case average:
